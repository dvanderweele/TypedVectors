const uint8ClampedVector = require("../../src/uint8ClampedVector.js");

QUnit.module("uint8ClampedVector");

QUnit.test("Instantiation.", assert => {
  const v1 = uint8ClampedVector(3, 2);
  assert.equal(v1.getSize(), 3, "size of 3");
  assert.equal(v1.getCapacity(), 3, "capacity of 3");
  assert.throws(()=>uint8ClampedVector(-1, 1));
...
QUnit.test("set method.", assert => {
  ...
  v1.set(1, -1);
  v1.set(2, 256);
  assert.equal(v1.at(1), 0);
  assert.equal(v1.at(2), 255);
});

QUnit.test("clamp method", assert => {
  const v1 = uint8ClampedVector();
  assert.equal(v1.clamp(), 0);
  assert.equal(v1.clamp(-1), 0);
  assert.equal(v1.clamp(256), 255);
});

QUnit.test("front method.", assert => {
  const v1 = uint8ClampedVector();
  const v2 = uint8ClampedVector(1,13);
...
QUnit.test("back method.", assert => {
  const v1 = uint8ClampedVector();
  const v2 = uint8ClampedVector(1, 7);
  const v3 = uint8ClampedVector(2, 3);
...
QUnit.test("doubleCapacity method.", assert => {
  ...
  const v2 = uint8ClampedVector(46, 2);
...
QUnit.test("pushBack method.", assert =>{
  const v1 = uint8ClampedVector(3, 3);
...
QUnit.test("shrinkToFit method.", assert => {
  const v1 = uint8ClampedVector(4, 4);
  v1.shrinkToFit();
  assert.equal(v1.getCapacity(), 4);
  v1.pushBack(5);
...
QUnit.test("reserve method.", assert => {
  ...
  const v2 = uint8ClampedVector(2, 13);
  v2.reserve(3);
...
QUnit.test("forEach method.", assert => {
  const v1 = uint8ClampedVector(4, 5);
  const gbv = [];
  const gbi = [];
  function cb(val, idx){
    gbv.push(val);
...
QUnit.test("iteration.", assert => {
  const v1 = uint8ClampedVector(3,13);
  const arr = [...v1];
...
QUnit.test("MIN and MAX.", assert => {
  const v1 = uint8ClampedVector(4);
  v1.set(0, v1.MIN);
  v1.set(1, v1.MAX);
  v1.set(2, (v1.MIN - 1));
  v1.set(3, (v1.MAX + 1));
  assert.equal(v1.at(0), 0);
  assert.equal(v1.at(1), 255);
  assert.notEqual(v1.at(2), -1);
  assert.notEqual(v1.at(3), 256);
});

QUnit.test("insert method.", assert => {
  const v1 = uint8ClampedVector(5, 5);
  ...
  const v2 = uint8ClampedVector(5, 5);
  v2.insert(2,10,3);
  assert.equal(v2.getCapacity(), 8);
  assert.equal(v2.getSize(), 8);
  assert.equal(v2.at(1), 5);
  assert.equal(v2.at(2), 10);
  assert.equal(v2.at(4), 10);
  assert.equal(v2.at(5), 5);
  assert.equal(v2.at(7), 5);
  assert.equal(v2.at(8), undefined);
  const v3 = uint8ClampedVector(5, 5);
  const v4 = uint8ClampedVector(5, 5);
  const v5 = uint8ClampedVector(5, 5);
  ...
  assert.equal(v3.at(0), 10);
  assert.equal(v3.at(1), 10);
  ...
  assert.equal(v5.at(5), 10);
  assert.equal(v5.at(6), 10);
  assert.equal(v5.at(7), undefined);
  const v6 = uint8ClampedVector(5, 5);
  const v7 = uint8ClampedVector(5, 5);
  const v8 = uint8ClampedVector(5, 5);
  ...
  const v9 = uint8ClampedVector(5, 5);
  const v10 = uint8ClampedVector(5, 5);
  const v11 = uint8ClampedVector(5, 5);
  const abf = new ArrayBuffer(2);
...
QUnit.test("filterInPlace method.", assert => {
  ...
  const v1 = uint8ClampedVector(5,0);
  v1.set(1,1);
  v1.set(2,2);
  v1.set(3,3);
  v1.set(4,4);
