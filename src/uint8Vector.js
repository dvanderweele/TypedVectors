module.exports = function uint8Vector(length = 0, value = 0) {
  // Argument Validation
	if (length < 0) {
		throw new Error(
			"uint8Vector can't be initialized to negative length."
		);
	}
  ...
  function insert(index = 0, value = 0, count = 1){
    ...
    if(index > size || index < 0){
      throw new Error("Value of 'index' passed to uint8Vector.insert must be greater than or equal to 0 and no greater than the size of the vector.");
    }
  ...
  function map(callback, preserveCapacity = true){
    const res = uint8Vector(preserveCapacity ? capacity : size, 0);
    for(let i = 0; i < size; i++){
  ...
  uint8VectorAPI[Symbol.iterator] = function *(){
    for(let i = 0; i < size; i++){
      yield view.getUint8(i);
    }
  };
  
  Object.freeze(uint8VectorAPI);
  
  return uint8VectorAPI;
}