module.exports = function int32Vector(length = 0, value = 0) {
  // Argument Validation
	if (length < 0) {
		throw new Error(
			"int32Vector can't be initialized to negative length."
		);
	}
  ...
  function getArrayBuffer(){
	...
	function at(index = 0){
	  ...
	  return view.getInt32(index * bpe);
	}
	function set(index = 0, value = 0){
	  if(index >= size || index < 0) return undefined;
	  view.setInt32(index * bpe, value);
    return null;
	}
	function front(){
	...
	function back(){
	  if(size === 0) return undefined;
	  return view.getInt32((size - 1) * bpe);
	}
	function pushBack(value){
	  if(size == capacity) doubleCapacity();
	  view.setInt32(size++ * bpe, value);
	}
	function popBack(){
	  if(size === 0) return undefined;
	  return view.getInt32((size-- - 1) * bpe);
    ...
    for(var i = 0; i < size; i++){
      tmpv.setInt32(i * bpe, view.getInt32(i * bpe));
    }
    buffer = tmpb;
    view = tmpv;
    capacity = newc;
  ...
  function shrinkToFit(){
    if(capacity > size){
      ...
      for(let i = 0; i < size; i++){
        tmpv.setInt32(i * bpe, view.getInt32(i));
      }
      capacity = size;
      buffer = tmpb;
      view = tmpv;
  ...
  function insert(index = 0, value = 0, count = 1){
    ...
    if(index > size || index < 0){
      throw new Error("Value of 'index' passed to int32Vector.insert must be greater than or equal to 0 and no greater than the size of the vector.");
    }
    ...
    if(typeof value == "number"){
      ...
      if(index === size){
        ...
        for(let i = size - 1; i >= index; i--){
          view.setInt32((i + count) * bpe, view.getInt32(i * bpe));
        }
        // insert value(s)
        for(let j = 0; j < count; j++){
          view.setInt32((index + j) * bpe, value);
        }
      }
      size += count;
      ...
      if(index === size){
        // simply append value(s)
        for(let i = 0; i < len; i++){
          view.setInt32((i + size) * bpe, value.getInt32(i * bpe));
        }
      } else {
        // shift value(s) that are to right of insertion point
        for(let i = size - 1; i >= index; i--){
          view.setInt32((i + len) * bpe, view.getInt32(i * bpe));
        }
        // insert value(s)
        for(let j = 0; j < len; j++){
          view.setInt32((index + j) * bpe, value.getInt32(j * bpe));
        }
      }
      size += len;
      ...
      if(index === size){
        // simply append value(s)
        for(let i = 0; i < len; i++){
          view.setInt32((i + size) * bpe, value[i]);
        }
      } else {
        // shift value(s) that are to right of insertion point
        for(let i = size - 1; i >= index; i--){
        ...
        // insert value(s)
        for(let j = 0; j < len; j++){
          view.setInt32((index + j) * bpe, value[j]);
        }
      }
      size += len;
      ...
      if(index === size){
        // simply append value(s)
        for(let i = 0; i < len; i++){
          view.setInt32((i + size) * bpe, tmpView.getInt32(i * bpe));
        }
      } else {
        // shift value(s) that are to right of insertion point
        for(let i = size - 1; i >= index; i--){
        ...
        // insert value(s)
        for(let j = 0; j < len; j++){
          view.setInt32((index + j) * bpe, tmpView.getInt32(j * bpe));
        }
      }
      size += len;
    } else {
    ...
    if(size > 0 && index >= 0 && index < size){
      ...
      if(index + quantity < size){
        for(let i = index + quantity; i < size; i++){
          ...
          view.setInt32(newIdx * bpe, view.getInt32(i * bpe));
        }
      }
  ...
  function forEach(callback){
    for(let i = 0; i < size; i++){
      callback(view.getInt32(i * bpe), i);
    }
  }
  function map(callback, preserveCapacity = true){
    const res = int32Vector(preserveCapacity ? capacity : size, 0);
    for(let i = 0; i < size; i++){
      res.set(i, callback(view.getInt32(i * bpe), i));
    }
    return res;
  }
  function mapInPlace(callback){
    for(let i = 0; i < size; i++){
      view.setInt32(i * bpe, callback(view.getInt32(i * bpe), i));
    }
  }
  function reduce(callback, initialValue = 0){
    for(let i = 0; i < size; i++){
      initialValue = callback(initialValue, view.getInt32(i * bpe), i);
    }
    return initialValue;
  }
  ...
  function filter(callback, preserveCapacity = true){
    ...
    let filterQty = 0;
    for(let i = 0; i < size; i++){
      if(callback(view.getInt32(i * bpe), i)){
        // keep current item, move if needed
        if(gapStart !== -1){
          res.set(gapStart, view.getInt32(i * bpe));
          gapStart++;
        } else {
          res.set(i, view.getInt32(i * bpe));
        }
        filterQty++;
      } else {
  ...
  function filterInPlace(callback){
    ...
    for(let i = 0; i < size; i++){
      if(callback(view.getInt32(i * bpe), i)){
        // keep current item, move if needed
        if(gapStart !== -1){
          view.setInt32(gapStart * bpe, view.getInt32(i * bpe));
          gapStart++;
        } 
      } else {
  ...
  int32VectorAPI[Symbol.iterator] = function *(){
    for(let i = 0; i < size; i++){
      yield view.getInt32(i * bpe);
    }
  };
  
  Object.freeze(int32VectorAPI);
  
  return int32VectorAPI;
}